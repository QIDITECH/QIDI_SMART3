...
extern bool page_wifi_list_ssid_button_enabled[3];
...
//2023.5.5 CLL gocde错误弹窗
extern std::string output_console;

//2023.5.10 CLL 修复页面反复跳转bug
...
void refresh_page_show() {
    ...
    */
    ...
    if (current_page_id != TJC_PAGE_RESET) {
        ...
        if (printer_webhooks_state == "shutdown") {
            if (current_page_id != TJC_PAGE_RESET || current_page_id != TJC_PAGE_POWER_OFF || current_page_id != TJC_PAGE_ABOUT_UPDATE || current_page_id != TJC_PAGE_ABOUT) {
        ...
        */
        ...
        // MKSLOG_RED("Printer idle timeout state: %s", current_webhooks_state_message.c_str());
        // sleep(1);
        if (printer_webhooks_state == "shutdown" || printer_webhooks_state == "error") {
            ...
            if (current_webhooks_state_message != printer_webhooks_state_message) {
                ...
                replace(temp.begin(), temp.end(), '\'', ' ');
                replace(temp.begin(), temp.end(), '\"', ' ');
                // boost::erase_all(current_webhooks_state_message, "\n");
                send_cmd_txt(tty_fd, "t0", temp);
                // MKSLOG_BLUE("跳到这里$$$$ %s", temp.c_str());
            }
        }
    ...
    /*
    if (current_page_id != TJC_PAGE_PREVEIW) {
        show_preview_complete = false;
    }
    */
    ...
    case TJC_PAGE_PRINT_FINISH:
        //2023.5.6 CLL 跳转至打印完成界面后只刷新一次时间
    ...
    */
    ...
    case TJC_PAGE_OPEN_LEVEL_INIT_3:
        refresh_page_open_level_init_3();
        break;
    ...
    case TJC_PAGE_OPEN_AUTO_LEVEL_2:
        refresh_page_open_auto_level_2();
        break;
    ...
    case TJC_PAGE_AUTO_INIT_1:
        refresh_page_auto_init_1();
        break;

    //2023.5.10 CLL 开机引导新增页面
    case TJC_PAGE_OPEN_HEATERBED:
        refresh_page_open_heaterbed();
        break;

    //2023.5.10 CLL 开机引导自动调平
    case TJC_PAGE_OPEN_LEVELING:
        refresh_page_open_leveling();
        break;
...
void refresh_page_open_auto_level_2() {
    std::string z_offset = std::to_string(printer_gcode_move_homing_origin[2] - 0.15);
    MKSLOG_RED("Z_offset: %s", z_offset.c_str());
...
void refresh_page_open_level_init_3() {
    ...
    if (printer_idle_timeout_state == "Ready") {
        ...
        page_to(TJC_PAGE_OPEN_AUTO_LEVEL_2);
    }
}
...
void refresh_page_open_filament_video_2() {
    ...
    send_cmd_txt(tty_fd, "t3", std::to_string(printer_extruder_temperature));
    send_cmd_txt(tty_fd, "t4", std::to_string(printer_extruder_target));
}

void refresh_page_open_compensat() {
    ...
    */
    ...
    if (auto_level_finished == true) {
        ...
        */
        ...
        init_mks_status();
        system("sync");
        if (TJC_PAGE_SAVING == current_page_id) {
...
void refresh_page_no_updata() {
    ...
    if (current_mks_oobe_enabled == false) {
        ...
        send_cmd_txt(tty_fd,"b7","");
    } else {
        send_cmd_picc(tty_fd, "b7", "157");
        send_cmd_txt(tty_fd,"b7","开机引导");
    }
    //}
...
//4.1.5 CLL 优化UI界面按钮显示
void refresh_page_auto_level() {
    if (auto_level_dist == (float)0.01) {
        send_cmd_picc(tty_fd, "b3", "103");
        send_cmd_picc2(tty_fd, "b3", "105");
        send_cmd_picc(tty_fd, "b4", "104");
        send_cmd_picc2(tty_fd, "b4", "106");
        send_cmd_picc(tty_fd, "b5", "104");
        send_cmd_picc2(tty_fd, "b5", "106");
        send_cmd_picc(tty_fd, "b6", "104");
        send_cmd_picc2(tty_fd, "b6", "106");
    } else if (auto_level_dist == (float)0.05) {
        send_cmd_picc(tty_fd, "b3", "104");
        send_cmd_picc2(tty_fd, "b3", "106");
        send_cmd_picc(tty_fd, "b4", "103");
        send_cmd_picc2(tty_fd, "b4", "105");
        send_cmd_picc(tty_fd, "b5", "104");
        send_cmd_picc2(tty_fd, "b5", "106");
        send_cmd_picc(tty_fd, "b6", "104");
        send_cmd_picc2(tty_fd, "b6", "106");
        ...
        send_cmd_picc(tty_fd, "b4", "104");
        send_cmd_picc2(tty_fd, "b4", "106");
        send_cmd_picc(tty_fd, "b5", "103");
        send_cmd_picc2(tty_fd, "b5", "105");
        send_cmd_picc(tty_fd, "b6", "104");
        send_cmd_picc2(tty_fd, "b6", "106");
        ...
        send_cmd_picc(tty_fd, "b5", "104");
        send_cmd_picc2(tty_fd, "b5", "106");
        send_cmd_picc(tty_fd, "b6", "103");
        send_cmd_picc2(tty_fd, "b6", "105");
    }
}
...
void refresh_page_stopping() {
    ...
    if (printer_idle_timeout_state == "Ready") {
        // sleep(2);
        //1.1.5 CLL 修复网页打印显示预览图bug
        clear_previous_data();
        sleep(5);
        //1.1.5 CLL 打印结束或取消时保存zoffset值
...
void refresh_page_set_zoffset_2() {
    ...
    //4.1.10 CLL 修改显示补偿值为最终值
    send_cmd_txt(tty_fd, "t1", std::to_string(printer_toolhead_position[2] - 0.15));
}
...
void refresh_page_set_zoffset() {
    send_cmd_txt(tty_fd, "t2", std::to_string(printer_gcode_move_homing_origin[2] - 0.15).substr(0,5) + "mm");
    if (auto_level_dist == (float)0.01) {
        send_cmd_picc(tty_fd, "b3", "98");
        send_cmd_picc2(tty_fd, "b3", "100");
        send_cmd_picc(tty_fd, "b4", "99");
        send_cmd_picc2(tty_fd, "b4", "101");
        send_cmd_picc(tty_fd, "b5", "99");
        send_cmd_picc2(tty_fd, "b5", "101");
        send_cmd_picc(tty_fd, "b6", "99");
        send_cmd_picc2(tty_fd, "b6", "101");
    } else if (auto_level_dist == (float)0.05) {
        send_cmd_picc(tty_fd, "b3", "99");
        send_cmd_picc2(tty_fd, "b3", "101");
        send_cmd_picc(tty_fd, "b4", "98");
        send_cmd_picc2(tty_fd, "b4", "100");
        send_cmd_picc(tty_fd, "b5", "99");
        send_cmd_picc2(tty_fd, "b5", "101");
        send_cmd_picc(tty_fd, "b6", "99");
        send_cmd_picc2(tty_fd, "b6", "101");
...
void refresh_page_manual_level() {
    send_cmd_txt(tty_fd, "t2", str_manual_level_offset);
}
...
void refresh_page_print_filament() {
    ...
    MKSLOG("printer_filament_extruedr_dist : %d", printer_filament_extruedr_dist);

    if (printer_filament_extruedr_dist == 10) {
        send_cmd_picc(tty_fd, "b0", "66");
        send_cmd_picc(tty_fd, "b1", "66");
        send_cmd_picc(tty_fd, "b2", "65");
    } else if (printer_filament_extruedr_dist == 50) {
        send_cmd_picc(tty_fd, "b0", "66");
        send_cmd_picc(tty_fd, "b1", "65");
        send_cmd_picc(tty_fd, "b2", "66");
    } else if (printer_filament_extruedr_dist == 100) {
        send_cmd_picc(tty_fd, "b0", "65");
        send_cmd_picc(tty_fd, "b1", "66");
        send_cmd_picc(tty_fd, "b2", "66");
    }
    */
    ...
    //2023.4.28 使网页打印显示预览图
    if (printer_print_stats_state == "complete") {
        send_cmd_txt(tty_fd, "t0", show_time((int)(printer_print_stats_print_duration))); //2023.5.6 CLL 跳转至打印完成界面后只刷新一次时间
        //1.1.5 CLL 修复网页打印显示预览图bug
        clear_previous_data();
        sleep(5);
        //1.1.5 CLL 打印结束或取消时保存zoffset值
...
void refresh_page_leveling_init() {
    ...
    if (printer_idle_timeout_state == "Ready") {
        ...
        {
        case TJC_PAGE_AUTO_LEVEL:
            MKSLOG_BLUE("跳到这里 %d", TJC_PAGE_AUTO_LEVEL);
    ...
    // MKSLOG_BLUE("Printer webhooks state: %s", printer_webhooks_state.c_str());
        ...
        if (printer_idle_timeout_state == "Printing") {
            ...
            if (page_filament_extrude_button == true) {
                send_cmd_vid_en(tty_fd, "gm1", 1);
            } else {
                send_cmd_vid_en(tty_fd, "gm1", 0);
            }
        } else {
            send_cmd_vid_en(tty_fd, "gm1", 0);
        ...
        */
        ...
        if (printer_out_pin_fan0_value == 0) {
            send_cmd_vid(tty_fd, "gm0", "4");
            send_cmd_picc2(tty_fd, "b18", "218");
        } else {
            send_cmd_vid(tty_fd, "gm0", "5");
            send_cmd_picc2(tty_fd, "b18", "219");
        }

        //1.1.6 CLL 新增fan3
        if (printer_out_pin_fan3_value == 0) {
            send_cmd_vid(tty_fd, "gm2", "15");
            send_cmd_picc2(tty_fd, "b17", "218");
        } else {
            send_cmd_vid(tty_fd, "gm2", "16");
            send_cmd_picc2(tty_fd, "b17" , "219");
        }

        if (mks_fila_status == true) {
        //if (filament_switch_sensor_fila_enabled == true) {
            send_cmd_picc(tty_fd, "b10", "220");
            send_cmd_picc2(tty_fd, "b10", "218");
        } else {
            send_cmd_picc(tty_fd, "b10", "221");
            send_cmd_picc2(tty_fd, "b10", "219");
        }
        if (printer_idle_timeout_state == "Printing") {
...
void refresh_page_printing_zoffset() {
    ...
    if (printer_pause_resume_is_paused == false) {
        send_cmd_picc(tty_fd, "b10", "52");
        send_cmd_picc2(tty_fd, "b10", "52");
    } else {
        send_cmd_picc(tty_fd, "b10", "64");
        send_cmd_picc2(tty_fd, "b10", "64");
    }
    ...
    //1.1.4 CLL 在打印设置zoffset界面检测到断料也会跳转至断料界面
    if (filament_switch_sensor_fila_enabled == true) {
...
void refresh_page_printing() {
    ...
    if (printing_keyboard_enabled == false) {
        ...
        send_cmd_txt(tty_fd, "t3", std::to_string(printer_heater_bed_temperature) + "/");
        send_cmd_val(tty_fd, "n1", std::to_string(printer_heater_bed_target));
        if (printer_heater_bed_target == 0) {
            send_cmd_pco(tty_fd, "n1", "65535");
            send_cmd_pco(tty_fd, "t3", "65535");
        ...
        if (printer_caselight_value == 0) {
            send_cmd_picc(tty_fd, "b0", "225");
            send_cmd_picc2(tty_fd, "b0", "224");
        } else {
            send_cmd_picc(tty_fd, "b0", "226");
            send_cmd_picc2(tty_fd, "b0", "223");
        }

        send_cmd_val(tty_fd, "n2", std::to_string((int)(printer_out_pin_fan0_value * 100)));
        //1.1.6 CLL 新增fan3
        send_cmd_val(tty_fd, "n3", std::to_string((int)(printer_out_pin_fan3_value * 100)));
        if (current_speed_factor != printer_gcode_move_speed_factor) {
            current_speed_factor = printer_gcode_move_speed_factor;
            send_cmd_val(tty_fd, "n4", std::to_string((int)(round(printer_gcode_move_speed_factor * 100))));
        }

        if (current_extruder_factor != printer_gcode_move_extrude_factor) {
            current_extruder_factor = printer_gcode_move_extrude_factor;
            send_cmd_val(tty_fd, "n5", std::to_string((int)(round(printer_gcode_move_extrude_factor * 100))));
        }
    } else {
        /*1.1.6 CLL 修复按钮显示bug
        if (printer_pause_resume_is_paused == false) {
    ...
    if (printer_print_stats_state == "complete") {
        ...
        save_current_zoffset();
        page_to(TJC_PAGE_PRINT_FINISH);
        send_cmd_txt(tty_fd, "t0", show_time((int)(printer_print_stats_print_duration))); //2023.5.6 CLL 跳转页面后自动清除原数据
    }
...
void refresh_page_preview() {
    ...
    if (printer_bed_leveling == true) {
        send_cmd_picc(tty_fd, "b3", "211");
        send_cmd_picc2(tty_fd, "b3", "211");
    }else {
        send_cmd_picc(tty_fd, "b3", "210");
        send_cmd_picc2(tty_fd, "b3", "210");
    }
...
void refresh_page_files_list_1() {
    for (int i = 0; i < 6; i++) {
        ...
        */
        ...
        //4.1.5 CLL 新增打印过文件会在文件列表第一页首个显示
        if (page_files_list_show_type[i] =="[c]") {
            send_cmd_picc(tty_fd, "b" + std::to_string(i+7), "228");
            send_cmd_picc2(tty_fd, "b" + std::to_string(i+7), "227");
        }
        if (page_files_list_show_type[i] == "[d]") {
            send_cmd_picc(tty_fd, "b" + std::to_string(i+7), std::to_string(44));
            send_cmd_picc2(tty_fd, "b" + std::to_string(i+7), std::to_string(46));
        } else if (page_files_list_show_type[i] == "[f]") {
            send_cmd_picc(tty_fd, "b" + std::to_string(i+7), std::to_string(45));
            send_cmd_picc2(tty_fd, "b" + std::to_string(i+7), std::to_string(47));
        } else if (page_files_list_show_type[i] == "[n]") {
            send_cmd_picc(tty_fd, "b" + std::to_string(i+7), "38");
            send_cmd_picc2(tty_fd, "b" + std::to_string(i+7), "38");
        }
    }
    if (page_files_current_pages == 0) {
        send_cmd_picc(tty_fd, "b0", std::to_string(40));
        send_cmd_picc2(tty_fd, "b0", std::to_string(40));
    } else {
        send_cmd_picc(tty_fd, "b0", std::to_string(39));
        send_cmd_picc2(tty_fd, "b0", std::to_string(39));
    }
    if (page_files_current_pages == page_files_pages) {
        send_cmd_picc(tty_fd, "b1", std::to_string(40));
        send_cmd_picc2(tty_fd, "b1", std::to_string(40));
    } else {
        send_cmd_picc(tty_fd, "b1", std::to_string(39));
        send_cmd_picc2(tty_fd, "b1", std::to_string(39));
    }
}
...
void refresh_page_files_list_2() {
    ...
    if (page_files_current_pages == 0) {
        send_cmd_picc(tty_fd, "b1", std::to_string(43));
        send_cmd_picc2(tty_fd, "b1", std::to_string(43));
    } else {
        send_cmd_picc(tty_fd, "b1", std::to_string(42));
        send_cmd_picc2(tty_fd, "b1", std::to_string(42));
    }
    if (page_files_current_pages == page_files_pages) {
        send_cmd_picc(tty_fd, "b2", std::to_string(43));
        send_cmd_picc2(tty_fd, "b2", std::to_string(43));
    } else {
        send_cmd_picc(tty_fd, "b2", std::to_string(42));
        send_cmd_picc2(tty_fd, "b2", std::to_string(42));
    }
}
...
void set_intern_zoffset(float offset) {
    printer_set_offset = offset;
     if (offset == (float)0.01) {
        send_cmd_picc(tty_fd, "b0", std::to_string(59));
        send_cmd_picc2(tty_fd, "b0", std::to_string(61));
        send_cmd_picc(tty_fd, "b1", std::to_string(60));
        send_cmd_picc2(tty_fd, "b1", std::to_string(62));
        send_cmd_picc(tty_fd, "b2", std::to_string(60));
        send_cmd_picc2(tty_fd, "b2", std::to_string(62));
        send_cmd_picc(tty_fd, "b3", std::to_string(60));
        send_cmd_picc2(tty_fd, "b3", std::to_string(62));
     } else if (offset == (float)0.05) {
        send_cmd_picc(tty_fd, "b0", std::to_string(60));
        send_cmd_picc2(tty_fd, "b0", std::to_string(62));
        send_cmd_picc(tty_fd, "b1", std::to_string(59));
        send_cmd_picc2(tty_fd, "b1", std::to_string(61));
        send_cmd_picc(tty_fd, "b2", std::to_string(60));
        send_cmd_picc2(tty_fd, "b2", std::to_string(62));
        send_cmd_picc(tty_fd, "b3", std::to_string(60));
        send_cmd_picc2(tty_fd, "b3", std::to_string(62));
        ...
        send_cmd_picc(tty_fd, "b1", std::to_string(60));
        send_cmd_picc2(tty_fd, "b1", std::to_string(62));
        send_cmd_picc(tty_fd, "b2", std::to_string(59));
        send_cmd_picc2(tty_fd, "b2", std::to_string(61));
        send_cmd_picc(tty_fd, "b3", std::to_string(60));
        send_cmd_picc2(tty_fd, "b3", std::to_string(62));
        ...
        send_cmd_picc(tty_fd, "b2", std::to_string(60));
        send_cmd_picc2(tty_fd, "b2", std::to_string(62));
        send_cmd_picc(tty_fd, "b3", std::to_string(59));
        send_cmd_picc2(tty_fd, "b3", std::to_string(61));
     }
}
...
void set_move_dist(float dist) {
    ...
    if (dist == (float)0.1) {
        send_cmd_picc(tty_fd, "b7", std::to_string(74));
        send_cmd_picc2(tty_fd, "b7", std::to_string(76));
        send_cmd_picc(tty_fd, "b8", std::to_string(75));
        send_cmd_picc2(tty_fd, "b8", std::to_string(77));
        send_cmd_picc(tty_fd, "b9", std::to_string(75));
        send_cmd_picc2(tty_fd, "b9", std::to_string(77));
    } else if (dist == (float)1.0) {
        send_cmd_picc(tty_fd, "b7", std::to_string(75));
        send_cmd_picc2(tty_fd, "b7", std::to_string(77));
        send_cmd_picc(tty_fd, "b8", std::to_string(74));
        send_cmd_picc2(tty_fd, "b8", std::to_string(76));
        send_cmd_picc(tty_fd, "b9", std::to_string(75));
        send_cmd_picc2(tty_fd, "b9", std::to_string(77));
        ...
        send_cmd_picc(tty_fd, "b8", std::to_string(75));
        send_cmd_picc2(tty_fd, "b8", std::to_string(77));
        send_cmd_picc(tty_fd, "b9", std::to_string(74));
        send_cmd_picc2(tty_fd, "b9", std::to_string(76));
    }
}
...
void finish_manual_level() {
    if ( manual_level_finished == false) {
        //2023.4.28 手动调平结束移动平台
        //ep->Send(json_run_a_gcode("SAVE_CONFIG"));
        ep->Send(json_run_a_gcode("G0 X0 Y0 Z50 F6000\nSAVE_CONFIG"));        all_level_saving = false;
        page_to(TJC_PAGE_SAVING);
    }
}
...
void set_manual_level_dist(float dist) {
    if (dist == (float)0.01) {
        send_cmd_picc(tty_fd, "b3", std::to_string(98));
        send_cmd_picc2(tty_fd, "b3", std::to_string(100));
        send_cmd_picc(tty_fd, "b4", std::to_string(99));
        send_cmd_picc2(tty_fd, "b4", std::to_string(101));
        send_cmd_picc(tty_fd, "b5", std::to_string(99));
        send_cmd_picc2(tty_fd, "b5", std::to_string(101));
        send_cmd_picc(tty_fd, "b6", std::to_string(99));
        send_cmd_picc2(tty_fd, "b6", std::to_string(101));
    } else if (dist == (float)0.05) {
        send_cmd_picc(tty_fd, "b3", std::to_string(99));
        send_cmd_picc2(tty_fd, "b3", std::to_string(101));
        send_cmd_picc(tty_fd, "b4", std::to_string(98));
        send_cmd_picc2(tty_fd, "b4", std::to_string(100));
        send_cmd_picc(tty_fd, "b5", std::to_string(99));
        send_cmd_picc2(tty_fd, "b5", std::to_string(101));
        send_cmd_picc(tty_fd, "b6", std::to_string(99));
        send_cmd_picc2(tty_fd, "b6", std::to_string(101));
        ...
        send_cmd_picc(tty_fd, "b4", std::to_string(99));
        send_cmd_picc2(tty_fd, "b4", std::to_string(101));
        send_cmd_picc(tty_fd, "b5", std::to_string(98));
        send_cmd_picc2(tty_fd, "b5", std::to_string(100));
        send_cmd_picc(tty_fd, "b6", std::to_string(99));
        send_cmd_picc2(tty_fd, "b6", std::to_string(101));
        ...
        send_cmd_picc(tty_fd, "b5", std::to_string(99));
        send_cmd_picc2(tty_fd, "b5", std::to_string(101));
        send_cmd_picc(tty_fd, "b6", std::to_string(98));
        send_cmd_picc2(tty_fd, "b6", std::to_string(100));
    }
...
void pre_manual_level_init() {
    ...
    if (start_pre_manual_level == false) {
        printer_idle_timeout_state = "Printing";
    }
    set_mks_babystep("-0.100");                  // 手动调平z_offset直接改为-0.1
    page_to(TJC_PAGE_LEVEL_INIT_2);
    ep->Send(json_run_a_gcode("G28\nABORT\nBED_MESH_CLEAR\n"));
    printer_idle_timeout_state = "Printing";
...
void move_tips() {
    ...
    if (current_page_id == TJC_PAGE_MOVE) {
        page_to(TJC_PAGE_POP_2);
    } else if (current_page_id == TJC_PAGE_PRINTING || current_page_id == TJC_PAGE_PRINT_BABYSTEP || current_page_id == TJC_PAGE_PRINT_FILAMENT) {
        cancel_print();
        page_to(TJC_PAGE_POP_2);
        ...
        page_to(TJC_PAGE_POP_2);
    } //2023.4.28 使坐标移动超出范围弹窗在所有界面都会弹出
}
...
void refresh_page_wifi_list() {
    for (int i = 0; i < 3; i++) {           //修改为每页三个wifi
        ...
        if (page_wifi_current_pages == 0) {
            if (strcmp(status_result.wpa_state, "COMPLETED") == 0) {
                send_cmd_picc(tty_fd, "wifi1", "124");
                page_wifi_list_ssid_button_enabled[0] = false;
            } else {
        ...
        if (page_wifi_ssid_list[i] == "") {
            ...
            send_cmd_picc(tty_fd, "wifi" + std::to_string(i + 1), "158");
            send_cmd_picc2(tty_fd, "wifi" + std::to_string(i + 1), "159");
            page_wifi_list_ssid_button_enabled[i] = false;
        } else {
            std::cout << "刷新为有锁的图片" << std::endl;
            send_cmd_picc(tty_fd, "wifi" + std::to_string(i + 1), "125");
            send_cmd_picc2(tty_fd, "wifi" + std::to_string(i + 1), "126");
            page_wifi_list_ssid_button_enabled[i] = true;
        }

        if (page_wifi_ssid_list_pages == 0) {
            MKSLOG_YELLOW("跳到这里");
            send_cmd_picc(tty_fd, "b7", "123");
            send_cmd_picc2(tty_fd, "b7", "123");
            send_cmd_picc(tty_fd, "b8", "123");
            send_cmd_picc2(tty_fd, "b8", "123");
        } else {
            if (page_wifi_current_pages == 0) {
                MKSLOG_YELLOW("跳到这里");
                send_cmd_picc(tty_fd, "b7", "123");
                ...
                MKSLOG_YELLOW("跳到这里");
                send_cmd_picc(tty_fd, "b7", "122");
                send_cmd_picc2(tty_fd, "b7", "127");
            }
            if (page_wifi_current_pages == page_wifi_ssid_list_pages - 1) {
                ...
                MKSLOG_YELLOW("跳到这里");
                send_cmd_picc(tty_fd, "b8", "122");
                send_cmd_picc2(tty_fd, "b8", "127");
            }
        }
    }
}
...
void set_print_filament_target() {
    if (0 == printer_extruder_target) {
        /* 20230201 1659 打印那边的耗材转载页面。预热温度和保存值不同，一直是240，应该随外面那个保存的预热值 */
        get_mks_extruder_target();
        set_extruder_target(mks_extruder_target);
    } else {
        set_extruder_target(0);
...
//2023.5.4 CLL 清除原数据
void back_to_main() {
    //2023.5.6 CLL 在所有界面均会跳转
    //if (current_page_id == TJC_PAGE_PRINT_STOPING || current_page_id == TJC_PAGE_PRINTING) {
    clear_previous_data();
    page_to(TJC_PAGE_MAIN);
    //}
}
...
void filament_pop_2_yes() {
    if (printer_extruder_temperature > 230) {
        filament_unload();
...
void open_more_level_finish() {
    ...
    //2023.5.12 CLL 修复调平完成后热床仍显示加热bug
    get_object_status();
    page_to(TJC_PAGE_MAIN);
}
...
void pre_open_auto_level_init() {
    ...
    send_var_value(tty_fd, "auto_level_step", 2);
    auto_level_finished = false;
    page_to(TJC_PAGE_OPEN_LEVEL_INIT_3);
    // int xtemp = printer_toolhead_axis_maximum[0] / 2 - printer_probe_x_zoffset;     // 直接用最大量程除以2
    // int ytemp = printer_toolhead_axis_maximum[1] / 2 - printer_probe_y_zoffset;     // 直接用最大量程除以2
    // ep->Send(json_run_a_gcode("BED_MESH_CLEAR\nG28\nG1 X" + std::to_string(xtemp) + " Y" + std::to_string(ytemp) + "F3600\nG1 Z15 F1200\nABORT\nPROBE_CALIBRATE\nTESTZ Z=-3.0\n"));
    //ep->Send(json_run_a_gcode("G28\nG1 X90 Y90 Z0.2\n"));
...
void refresh_page_zoffset() {
    ...
    for (int i = 0; i < printer_bed_mesh_profiles_mks_mesh_params_y_count; i++) {
        ...
        for (int j = 0; j < printer_bed_mesh_profiles_mks_mesh_params_x_count; j++) {
            ...
            send_cmd_txt(tty_fd, "t" + std::to_string(4 * i + j), temp[i][j]);
        }
    }
    /*
    std::string temp[16];
...
//2023.5.3 修改自动调平页面跳转
void refresh_page_auto_init_1() {
    send_cmd_txt(tty_fd, "t1", std::to_string(printer_heater_bed_temperature) + "/");
    send_cmd_val(tty_fd, "n1", std::to_string(printer_heater_bed_target));
    if (printer_heater_bed_target > 0) {
        send_cmd_picc(tty_fd, "b2", "186");
        send_cmd_picc2(tty_fd, "b2", "184");
        send_cmd_pco(tty_fd, "t1", "63488");
        send_cmd_pco(tty_fd, "n1", "63488");
    } else {
        send_cmd_picc(tty_fd, "b2", "187");
        send_cmd_picc2(tty_fd, "b2", "183");
        send_cmd_pco(tty_fd, "t1", "65535");
        send_cmd_pco(tty_fd, "n1", "65535");
    }
}
...
//2023.5.8 CLL 新增开机引导界面
void open_heater_bed_up() {
    page_to(TJC_PAGE_OPEN_VIDEO_4);
    //4..1.9 CLL 修改开机引导平台移动距离
    ep->Send(json_run_a_gcode("G91\nG1 Z-30 F600\nG1 X-30 Y-30 F1200\nG90\n"));
    ep->Send(json_run_a_gcode("SET_KINEMATIC_POSITION Z=90\n"));
}

//2023.5.10 CLL 新增开机引导页面
void refresh_page_open_heaterbed() {
    ...
    if (printer_heater_bed_target > 0) {
        send_cmd_picc(tty_fd, "b3", "198");
        send_cmd_picc2(tty_fd, "b3", "197");
        send_cmd_pco(tty_fd, "t0", "63488");
        send_cmd_pco(tty_fd, "n0", "63488");
    } else {
        send_cmd_picc(tty_fd, "b3", "199");
        send_cmd_picc2(tty_fd, "b3", "196");
        send_cmd_pco(tty_fd, "t0", "65535");
        send_cmd_pco(tty_fd, "n0", "65535");
    }
}
...
void refresh_page_open_leveling() {
    if ((printer_idle_timeout_state == "Ready" || printer_idle_timeout_state == "Idle")) {
    // if ((printer_idle_timeout_state == "Ready" || printer_idle_timeout_state == "Idle") && printer_webhooks_state == "Ready") {
        page_to(TJC_PAGE_OPEN_LEVEL_FINISH);
        auto_level_enabled = false;
    }
}
...
void save_current_zoffset() {
    ...
    case TJC_PAGE_SET_ZOFFSET:
        printer_idle_timeout_state = "Printing";
        page_to(TJC_PAGE_SAVING);
        ep->Send(json_run_a_gcode("G0 X0 Y0 F5000\n G0 Z50 F600\n"));
        ep->Send(json_run_a_gcode("SAVE_CONFIG\n"));
        z = std::stof(z_offset);
        z = z - 0.15;
        if (z > -5 && z < 5) {
