...
std::string page_files_list_name[6] = {"", "", "", "", "", ""};                                     // 文件列表显示文件名称
std::string page_files_list_show_name[6] = {"", "", "", "", "", ""};                                // 文件列表名称
std::string page_files_list_show_type[6] = {"[n]", "[n]", "[n]", "[n]", "[n]", "[n]"};        // 文件类型: [f]或者[d]，或者[n]
...
void get_page_files_filelist(std::string current_dir) {
    ...
    if (0 == page_files_dirname_filename_list.size() % 6) {
        page_files_pages = page_files_dirname_filename_list.size() / 6 - 1;
        //2023.5.9 CLL 修复在空文件夹下一页按钮仍亮起
        if (page_files_pages <= 0) {
            page_files_pages = 0;
        }
    } else {
        page_files_pages = page_files_dirname_filename_list.size() / 6;
    }
}
...
void get_parenet_dir_files_list() {
    ...
    //1.1.6 CLL 使预览图界面按下返回按钮后处于该文件所在页面
    if (current_page_id != TJC_PAGE_PREVIEW) {
        page_files_current_pages = 0;
    }
    //1.1.6 CLL 在预览图或文件界面拔出U盘后防卡死
    if (detect_disk() == -1) {
        page_files_pages = 0;
        page_files_current_pages = 0;
